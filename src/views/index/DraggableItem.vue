...
let components = {
  itemBtns(h, element, index, parent) {
    let {copyItem, deleteItem} = this.$listeners
...
layouts = {
  ...
  rowFormItem(h, element, index, parent) {
    let {activeItem} = this.$listeners,
      className = this.activeId == element.formId ? 'drawing-row-item active-from-item' : 'drawing-row-item'
    return (
...
export default {
  ...
  render(h) {
    let layout = layouts[this.element.layout]
